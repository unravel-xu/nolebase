...
& =E_\pi\left[R_{t+1} \mid S_t=s, A_t=a\right]+\gamma E_{\pi}\left[G_{t+1}\mid S_{t} = s, A_{t}=a\right] \\
& =R(s,a)+\gamma \left(\sum G_{t+1} \cdot P(G_{t+1}|S_t=s, A_t=a)\right) \\
& =R(s,a)+\gamma \left[\sum G_{t+1} \cdot \left(P(G_{t+1}|S_{t+1}=s')\cdot P(S_{t+1}=s'|S_t=s,A_t=a)\right)\right]
\end{aligned}
$$