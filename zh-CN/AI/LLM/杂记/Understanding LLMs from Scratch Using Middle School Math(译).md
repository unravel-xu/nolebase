...
我们知道嵌入就是把字符转化为向量，以方便语言模型更好的处理

但使用单个字符作为语言的基本构建块有局限性：神经网络需要承担繁重的任务，它们需要理解一个字符序列（即单词）在彼此之间、以及与其他单词之间的关系

如果不用字符而是直接为单词分配嵌入向量，让神经网络预测下一个单词会怎样呢？比如可以为每个单词（如“I”、“love”、“you”等）分配一个多维向量，然后将这些单词的向量输入神经网络，神经网络则负责预测下一个单词。这样会不会更好？

> [!important] 用 token 表示嵌入的基本单位

之前模型使用单个字符作为 token，现在我们提出使用整个单词作为 token

比如处理"cat"这个词，之前模型需要分别处理 c、a、t 三个字符的向量。这种方式看似简单，却让模型承担了过重的负担——它必须自行发现"c-a-t"组合起来代表猫，还要理解"c-a-t-s"中的 s 表示复数。举个具体例子，当模型看到这两个句子时：
...
1. cat 和 cats 共享前三个字符
2. sat 和 ate 都包含 at 但含义不同

现在尝试改用整词分词：为每个单词（如 cat 、cats）分配独立向量。这显然可以大大简化学习任务！但也带来了新问题——比如英语有超过 18 万个单词，中文常用字词也超过大几万。想象一个需要管理 10 万种“文字基因”的模型，光是存储这些向量就要消耗海量资源！
...
- $unhappy \to un\ , \ happy$
- $cats \to cat\ , \ s$
- $中文分词 \to 中\ , \ 文\ , \ 分\ , \ 词$（注：实际处理会更智能）

我们将一个 token 拆成多个 token，这种拆解带来了三重优势：
...
2. 应对生僻词
   ...
   遇到"ChatGPT"这种新造词时，子词分词可以将其拆解为Chat，G，P，T，模型立即明白这与聊天程序相关。传统方法遇到新词就只能瞎猜

3. 平衡效率与效果
   
   主流子词分词器（如 BERT 用的 WordPiece、GPT 用的 BPE）通常只保留 3-5 万个常用子词。对比整词分词的 18 万词库，存储量减少 80%以上，却覆盖了更广泛的语言现象。

实际案例：
OpenAI 的 GPT-3 使用 BPE 分词器后，即便面对网络俚语“LOLcats”也能游刃有余地拆解为LOL，cat，s，既理解这是搞笑猫图，又保持语法正确性
...
1. 如何智能切割？
   ...
   以“underground”为例，可能有多种切分方式：
	- under + ground（在地下）
	- un + der + ground（不接地）

   分词器需要根据上下文选择正确拆分方式，这依赖统计学习：如果语料库中“under + ground”经常一起出现，就优先保留这种拆分

2. 中文分词的特色

   不同于英文的空格分词，中文分词更具挑战。优秀的分词器应该能区分：

	- 武汉市长江大桥 $\to$ 武汉，市，长江，大桥（✔）
	- 武汉市长江大桥 $\to$ 武汉，市长，江大桥（✘）

3. 统一编码方案
   
   现代分词器（例如 SentencePiece）采用统一编码：

	- 将文本转换为 Unicode 字符
	- 统计高频字符组合，逐步合并为子词
	- 最终形成包含单字、词语、词缀的混合词表

分词（tokenize）有三种粒度：字符级（char）/ 词语级（word）/ 子词级（subword）

分词器将输入文本按粒度拆成一系列 token 并返回相应的数字，用于查找该 token 在嵌入矩阵中对应的向量

当我们把嵌入（embeddings）和子词分词（subword tokenization）结合起来，一个模型可能是下面这样的：

![[pic-20250411163425502.png]]
